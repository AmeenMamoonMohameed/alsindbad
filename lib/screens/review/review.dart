...
class _ReviewState extends State<Review> {
  final _reviewCubit = ReviewCubit();
  ...
  Future<void> _onScroll() async {
    ...
    final state = _reviewCubit.state;
    if (state is ReviewSuccess && state.canLoadMore) {
      await _reviewCubit.onLoadMore();
    }
  }

  ///On refresh
  Future<void> _onRefresh() async {
    await _reviewCubit.onLoad();
  }
  ...
  Future<void> _onAction(CommentModel item, BuildContext context) async {
                ...
                child: Column(
                  children: [
                    ...
                    Column(
                      children: [
                        orderItem,
                        if (item.reviewType == ReviewType.review &&
                            (item.replaies != null &&
    ...
    if (result == 'report') {
      if (AppBloc.userCubit.state == null) {
        final result = await Navigator.pushNamed(
          context,
          Routes.signIn,
          arguments: Routes.productReview,
        );
        if (result != Routes.productReview) {
          return;
        }
      }
      _onReport(
    ...
    if (result == 'remove') {
      var result = await _reviewCubit.onRemove(
